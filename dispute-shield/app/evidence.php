...
function buildActivityLog(array $u, string $email): string {
    ...
    if ((int)$u['no_connects_events'] > 0) {
        $lines[] = "  Auto-bidder pauses (no connects):  " . (int)$u['no_connects_events'] .
                   "x  [NOTE: paused due to zero Upwork Connects balance in agency account - NOT a GigRadar failure]";
    }
...
function buildRebuttalLetter(array $u, string $email, string $reason): string {
    ...
    if (in_array($stripeStatus, ['canceled', 'cancelled'])) {
        $cancelDate = $subCancel ?: 'unknown date';
        $out[] = "Subscription status:  CANCELED (confirmed via Stripe)";
    } elseif ($u['is_canceled'] && $subCancel) {
        $out[] = "Subscription status:  Canceled by customer on $subCancel";
    } elseif ($stripeStatus === 'active') {
        $out[] = "Subscription status:  ACTIVE (confirmed via Stripe)";
    } elseif ($stripeStatus) {
        $out[] = "Subscription status:  " . strtoupper($stripeStatus) . " (Stripe)";
    } else {
        $out[] = "Subscription status:  ACTIVE";
    }
    ...
    if ($noConn > 0) {
        ...
        $out[] = "   The auto-bidder paused $noConn time(s) due to insufficient";
        $out[] = "   Upwork Connects balance on the customer's agency account.";
        $out[] = "   This is NOT a GigRadar service failure.";
        $out[] = "";
        $out[] = "   IMPORTANT CONTEXT:";
        $out[] = "   * \"Connects\" = Upwork's own virtual currency used to submit";
        $out[] = "     proposals. They are purchased directly from Upwork by the";
        $out[] = "     agency and stored in the agency's Upwork balance.";
        $out[] = "   * GigRadar cannot bid when the agency's Upwork Connects";
        $out[] = "     balance reaches zero. This is an Upwork platform limitation.";
        $out[] = "   * GigRadar's platform was fully operational at all times.";
        $out[] = "   * The customer was notified each time the auto-bidder paused.";
        $out[] = "   * Despite temporary pauses, customer sent $proposals proposals";
        $out[] = "     total - proving the service worked effectively when connects";
        $out[] = "     were available.";
        $out[] = "   * Analogy: a printer cannot print when it runs out of paper.";
        $out[] = "     The software (GigRadar) is not at fault for the paper supply.";
        $out[] = "";
        $n++;
    }
